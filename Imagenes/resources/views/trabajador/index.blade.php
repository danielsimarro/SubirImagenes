...
<a href="{{ url('imagen') }}" class="btn btn-success">Ver tabla de imagenes</a>
        ...
        @foreach ($trabajadores as $trabajador)
            <tr>
                ...
                <td>
                    ...
                    <?php $exists = file_exists( 'upload/' . $trabajador->id );?>
    
                    @if($exists == 1)
                    <!--Mostrar imagen en el html-->
                        <td>
                            <img src="{{ url('upload/' . $trabajador->id ) }}">
                        </td>
                     @else
                   <td>Este trabajador no tiene imagen</td>
                 
                   @endif
                    
                
                <td>
                    <a href="{{ url('trabajador/' . $trabajador->id .  '/imagen') }}" class="btn btn-success">Subir imagen</a>
                    <input type="hidden" name="puesto" value="{{ $trabajador->id }}"/>
